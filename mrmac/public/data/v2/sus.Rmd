...
bxp <- ggplot(df, aes(x = role, y = sus_score)) + 
  ...
  theme(
    ...
    strip.background = element_blank(),
    plot.background = element_blank(),
    plot.margin = unit(c(0.005, .1, 0, 0), "null"),
    panel.border = element_blank(),
...
stats <- boxplot_data %>%
  wilcox_test(value~C, p.adjust.method = 'holm', paired = TRUE) %>%
  add_significance("p") %>%
  add_xy_position(x = "C")

# Plot the box plot
bxq <- ggplot(boxplot_data, aes(x = C, y = value)) +
  geom_boxplot() +
  ...
  scale_color_manual(name = "conditions", values = coloring) +
  stat_pvalue_manual(stats, label = 'p.adj.signif', hide.ns = TRUE, tip.length = 0.02) +
  scale_y_continuous(minor_breaks = seq(200, 650, 100), breaks = seq(200, 650, by=100), limits=c(200, 650)) +
  annotation_custom(grid::linesGrob(y = c(0,0), gp = grid::gpar(lwd=3))) +
  xlab("(b)") +
  ylab("Time (seconds)") +
  theme_minimal()+
...
final_plot <- grid.arrange(bxp, bxq, nrow = 1, ncol = 2, widths = c(2, 1))

# saving the final figure
ggsave("out/usability.pdf", final_plot, width = 8, height =4, dpi = 1000)
...
random_numbers <- rnorm(n, mean = mean_value, sd = std_value)
...
std_random <- sd(random_numbers)

cat("Mean:", mean_random, "\n")
cat("Standard Deviation:", std_random, "\n")
