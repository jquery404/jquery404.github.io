...
# Add facet grid
p <- p + facet_grid(rows = vars(qus), switch = "y") + xlab("Preference questions") +
...
p <- p + geom_signif(comparisons = list(c("C1", "C2"), c("C2", "C3")), 
              annotations = c("*", "**"), 
              y_position = 36.5, 
              textsize = 4) 

# Add theme elements
p <- p + theme(axis.text.y = element_text(face = "bold", color = "black"),
