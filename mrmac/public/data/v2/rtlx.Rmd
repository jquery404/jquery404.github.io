...
c <- cp(df_tm, dodge, 'Temporal') + theme(axis.ticks = element_blank())
d <- cp(df_ef, dodge, 'Effort') + theme(axis.text.y = element_text(color="black", size=fontsize), 
                                        plot.margin = unit(c(0.0, .025, 0, 0), "null"))
e <- cp(df_fr, dodge, 'Frustration') + theme(axis.ticks.y = element_blank())
f <- cp(df_pf, dodge, 'Performance') + theme(axis.ticks.y = element_blank(), plot.margin = unit(c(0.0, .025, 0, 0), "null"))
g <- cp(df_ov, dodge, 'Overall') + theme(axis.ticks.y = element_blank(), plot.margin = unit(c(0.0, .025, 0, 0), "null"))
...
ggsave("out/rtlx.pdf", final_plot, width = 6.65, height = 5, dpi = 1000)
```
