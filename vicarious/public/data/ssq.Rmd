...
condition_order <- c("NS-pre", "NS-post", "GS-pre", "GS-post", "MS-pre", "MS-post", "AS-pre", "AS-post")
...
df %>%
  ...
  shapiro_test(total)
qqnorm(df$total);qqline(df$total)
ggqqplot(df, "total", ggtheme = theme_bw()) + facet_grid(conditions~role)

ggsave("out/ssq_ggqq.pdf", width = 8, height = 8, dpi = 1000)  
```
...
m = art(total ~ conditions * role + (1|id), data = df)
anova(m)
...
custom_boxplot <- function(df, means, dodge, fontsize, coloring, tag) {
  ...
  scale_y_continuous(minor_breaks = seq(0, 15, 5), breaks = seq(0, 15, by=5), limits=c(0, 15)) +
