...
qqnorm(df$score);qqline(df$score)

ggqqplot(df, "score", ggtheme = theme_bw()) + facet_grid(params~conditions)
...
m = art(score ~ conditions * role + (1|id), data = df)
anova(m)

shapiro.test(residuals(m))
qqnorm(residuals(m));qqline(residuals(m))

emmeans(artlm(m, "conditions"), pairwise ~ conditions)
emmeans(artlm(m, "role"), pairwise ~ role)
...
df=read.csv('csv/spatial.csv', sep=",")
avg_df <- aggregate(score ~ grp + id + params + role + conditions, data = df, FUN = mean)
...
ggqqplot(avg_df, "score", ggtheme = theme_bw()) + facet_grid(conditions~role)

# ggsave("out/spatial_ggqq.pdf", width = 8, height = 8, dpi = 1000)  

```

```{r}
res.aov <- anova_test(
  data = avg_df, dv = score, wid = id,
